...
export function createPolicy(index: number): PolicyBundle {
  ...
  if (index === 0) {
    ...
    return {
      policy: { id, name: "Acme Employee Wellness Policy FY2026", code: "BEN-STD-01", version: "V1.1", description: "Core employee wellness handbook policy for full-time staff coverage.", organizationId: org.orgId, eligibleEmploymentTypes: ["full-time"], dependentCoverages: [{ type: "spouse" }, { type: "child" }, { type: "mother" }, { type: "father" }, { type: "sibling" }], dependentsPoolType: "SharedWithEmployee", benefitPoolType: "Individual", utilisationMode: "Fixed", refreshCycle: "Yearly", refreshStartReference: "financial_year", status: "active", groupCount: groups.length, orgName: org.orgName, createdAt: "2026-01-15T10:00:00Z" },
  ...
  if (index === 1) {
    ...
    return {
      policy: { id, name: "Acme Leadership Benefits Policy FY2026", code: "BEN-EXC-02", version: "V2.0", description: "Leadership-tier benefits handbook policy with expanded clinical therapy coverage.", organizationId: org.orgId, eligibleEmploymentTypes: ["full-time", "part-time"], dependentCoverages: [], benefitPoolType: "Individual", utilisationMode: "Prorated", prorateUnit: "Monthly", refreshCycle: "Quarterly", refreshStartReference: "financial_year", status: "draft", groupCount: groups.length, orgName: org.orgName, createdAt: "2026-03-22T14:30:00Z" },
      data: { groups, benefits },
    }
  }
  ...
  if (index === 2) {
    ...
    return {
      policy: { id, name: "Global Tech Core Benefits Policy FY2026", code: "BEN-CON-03", version: "V1.0", description: "Core logistics workforce handbook policy for contract and internship coverage.", organizationId: org.orgId, eligibleEmploymentTypes: ["contract", "internship"], dependentCoverages: [], benefitPoolType: "Individual", utilisationMode: "Fixed", refreshCycle: "Yearly", refreshStartReference: "financial_year", status: "deactivated", groupCount: groups.length, orgName: org.orgName, createdAt: "2026-11-05T09:15:00Z" },
      data: { groups, benefits },
    }
  }
  ...
  if (index === 4) {
    ...
    return {
      policy: { id, name: "Acme Senior Band Wellness Supplement FY2026", code: "BEN-STD-01-SNR", version: "V1.1", description: "Senior band supplement with elevated therapy cap versus base policy.", parentPolicyId: parentId, organizationId: org.orgId, eligibleEmploymentTypes: ["full-time"], dependentCoverages: [], benefitPoolType: "Individual", utilisationMode: "Fixed", refreshCycle: "Yearly", refreshStartReference: "financial_year", status: "active", groupCount: groups.length, orgName: org.orgName, targetEmployeeIds: ["EMP-20260115-0002", "EMP-20260115-0004"], createdAt: "2026-02-15T00:00:00Z" },
      data: { groups, benefits },
    }
  }
  ...
  return {
    policy: { id, name: `Wellness Policy ${n}`, code: `BEN-GEN-0${n}`, description: `Generated policy for ${org.orgName}.`, organizationId: org.orgId, eligibleEmploymentTypes: ["full-time"], dependentCoverages: [], benefitPoolType: "Individual", utilisationMode: modeOpts[g]!, prorateUnit: modeOpts[g] === "Prorated" ? "Monthly" : undefined, refreshCycle: cycleOpts[g]!, refreshStartReference: "financial_year", status: statusOpts[g]!, groupCount: 1, orgName: org.orgName, createdAt: "2026-04-01T00:00:00Z" },
    data: { groups, benefits },
  }
}
