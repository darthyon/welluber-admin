...
export default function MembersPage() {
  ...
  const filteredMembers = useMemo(() => {
    return MOCK_MEMBERS.filter((member) => {
      ...
      const matchesOrg =
        orgFilter === "all" || member.organization.name === orgFilter
      const matchesBranch =
        branchFilter === "all" || member.branch?.name === branchFilter
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="cursor-pointer text-body font-medium text-foreground transition-colors hover:text-primary">
              {row.name}
            </span>
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-body font-normal text-subtle">
            Record of global workforce members, including employees and their
            dependents who have signed up on the app. Track activation status
            and member activity.
          </p>
        </div>
        <div className="flex items-center gap-2">
