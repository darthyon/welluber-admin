...
function NewPolicyForm({ organizationId }: { organizationId?: string }) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { templates: policyTemplates, isLoading: templatesLoading } = usePolicyTemplates()
  const source = organizationId ? "org" : searchParams.get("source") ?? (searchParams.get("orgId") ? "org" : "global")
  const templateId = searchParams.get("template")
  const cloneId = searchParams.get("clone")
  const paramOrgId = organizationId ?? searchParams.get("orgId")
  const backHref =
    source === "org" && paramOrgId
      ? `/organizations/${paramOrgId}?tab=policies`
      : "/policies"

  const [policyStep, setPolicyStep] = useState<1 | 2 | 3>(1)
  ...
  return (
    <div className="animate-in pb-24 duration-500 fade-in slide-in-from-bottom-4">
      <div className="mx-auto flex max-w-[1280px] flex-col items-start gap-12 lg:flex-row lg:gap-16">
        <NewPolicySidebar issues={issues} jumpToIssue={jumpToIssue} targeting={targeting} />
        ...
        <div className="w-full min-w-0 flex-1">
          <div className="flex flex-col gap-6">
            ...
            <NewPolicyPageActionBar
              currentStep={policyStep}
              onBack={() => setPolicyStep((step) => Math.max(1, step - 1) as 1 | 2 | 3)}
              onCancel={() => guardedLeave(() => router.push(backHref))}
              onNext={() => setPolicyStep((step) => Math.min(3, step + 1) as 1 | 2 | 3)}
            />
          </div>
        </div>
      </div>
...
export function NewPolicyCreateFlow({ organizationId }: { organizationId?: string }) {
  return (
    <Suspense fallback={<div data-test="fallback">FALLBACK</div>}>
      <NewPolicyForm organizationId={organizationId} />
    </Suspense>
  )
}
