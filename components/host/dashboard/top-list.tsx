    ...
    <div className="rounded-lg border border-border bg-card p-5 h-full flex flex-col">
      <div className="flex items-start justify-between mb-4 gap-2">
        ...
        <select 
          ...
          className="px-2 py-1 bg-muted/50 border border-border hover:bg-muted/80 rounded text-[11px] font-medium text-foreground outline-none cursor-pointer shrink-0 transition-colors"
        >
          {type === "org" ? (
            <>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.id} className="group border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors">
                <td className="py-2.5 text-[12px] text-muted-foreground/80 font-mono">
                ...
                <td className="py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] font-medium text-foreground truncate max-w-[120px]">{item.name}</span>
                  </div>
                </td>
                <td className="py-2.5 text-[13px] text-right text-foreground">{item.metric1}</td>
                <td className="py-2.5 text-[12px] font-medium text-right text-muted-foreground">
                  {type === "org" ? (
                    <span className="inline-flex py-0.5 px-1.5 bg-muted/50 rounded text-muted-foreground/80 text-[11px]">{item.metric2}</span>
                  ) : (
