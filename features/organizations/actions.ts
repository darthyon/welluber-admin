...
export async function createOrganization(data: CreateOrganizationData) {
  ...
  // Follow the seed convention (ORG-NEW-YYYYMMDD-XXXX) so the id reads naturally in the UI.
  const now = new Date()
  const datePart = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, "0")}${String(now.getDate()).padStart(2, "0")}`
  const mockId = `ORG-NEW-${datePart}-${Date.now().toString().slice(-4)}`
...
export async function createHqBranch(orgId: string, data: HqBranchData) {
  ...
  return {
    success: true,
    data: {
      id: `br_${Date.now().toString().slice(-6)}`,
      ...data,
      type: "hq",
      orgId,
    },
...
export async function assignPoliciesToOrganization(
  orgId: string,
  policyIds: string[]
) {
  await new Promise((resolve) => setTimeout(resolve, 500))
...
export async function sendAccountPaymentLink(
  orgId: string,
  amount: number,
  financeEmail: string
) {
